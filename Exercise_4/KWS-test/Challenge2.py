...
def read_svg_file(filenumber):
    with open('Exercise_4/KWS-test/locations/' + str(filenumber) + '.svg', 'r') as f:
        svg_data = f.read()
...
def get_images_from_words(filenumber, words):
    img = Image.open('Exercise_4/KWS-test/images/' + str(filenumber) + '.jpg')
...
def read_keywords():
    f = open('Exercise_4/KWS-test/keywords.txt', 'r')
    Lines = f.readlines()
...
train_files = []
validation_files = []
train_files.extend(range(305, 309))
validation_files.extend(range(305, 309))
